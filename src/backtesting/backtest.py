...
# Grades para otimização de thresholds
DEFAULT_BUY_GRID = np.arange(0.05, 1.00, 0.05)
DEFAULT_SELL_GRID = np.arange(-0.80, 0.80, 0.05)
EXPANDED_BUY_GRID = np.arange(0.01, 0.99, 0.02)
EXPANDED_SELL_GRID = np.arange(-0.90, 0.90, 0.02)
SCORE_BUY_GRID = np.arange(-0.50, 0.80, 0.02)
SCORE_SELL_GRID = np.arange(-0.80, 0.50, 0.02)
...
def calculate_ml_metrics(y_true_multiclass, y_pred_actions):
    """
    Calcula métricas de ML comparando ações 0/1 com rótulo binário up-vs-not-up.
    
    Args:
        y_true_multiclass: Array com classes multiclasse (0=down, 1=flat, 2=up)
        y_pred_actions: Array com ações de trading (0=flat, 1=long)
...
def generate_actions_from_score(score_series, buy_threshold=0.25, sell_threshold=-0.05, minimum_hold_days=3):
    ...
    Args:
        score_series: Array com scores
        buy_threshold: Threshold para compra
        sell_threshold: Threshold para venda
        minimum_hold_days: Mínimo de dias entre mudanças
        
    Returns:
        Array com ações de trading
    """
    trading_actions = []
    current_position = 0
...
def generate_actions_from_prob(prob_series, buy_threshold=0.6, sell_threshold=0.4, minimum_hold_days=3, gate_threshold=None):
    ...
    Args:
        prob_series: Array com probabilidades
        buy_threshold: Threshold para compra
        sell_threshold: Threshold para venda
        minimum_hold_days: Mínimo de dias entre mudanças
...
def compute_up_down_score_from_proba(probabilities_matrix):
    ...
    Args:
        probabilities_matrix: Array com probabilidades multiclasse
        
    Returns:
        Array com scores calculados
    """
...
def simulate_portfolio_execution_next_open(test_dataframe, trading_actions, initial_capital, transaction_cost_percentage):
    """
    Simula execução de portfólio com execução no próximo pregão (Open t+1).
    
    Args:
        test_dataframe: DataFrame com dados OHLCV
        trading_actions: Array 0/1 representando posição desejada
        initial_capital: Capital inicial
        transaction_cost_percentage: Custo de transação como percentual
        
    Returns:
        Série com histórico do valor do portfólio
    """
...
def calculate_position_fraction(probability, sizing_config):
    """
    Calcula a fração do capital a ser investida baseada na probabilidade.
    ...
    Returns:
        Fração do capital a ser investida (0 a 1)
    """
...
def calculate_max_drawdown(portfolio_value_series):
    ...
    Returns:
        Maximum Drawdown como percentual (0 a 1)
    """
...
def adaptive_threshold_optimization_with_probs(probabilities, validation_dataframe, initial_capital, transaction_cost_percentage, minimum_hold_days=3, gate_threshold=None):
    """
    Otimiza thresholds usando probabilidades pré-calculadas.
    
    Args:
        probabilities: Array de probabilidades multiclasse
        validation_dataframe: DataFrame OHLCV para simulação
        initial_capital: Capital inicial
        transaction_cost_percentage: Custo de transação
        minimum_hold_days: Mínimo de dias entre mudanças
        gate_threshold: Threshold de confiança opcional
        
    Returns:
        Tupla com (best_buy_threshold, best_sell_threshold, best_sharpe_ratio)
    """
...
def adaptive_threshold_optimization_with_score(probabilities, validation_dataframe, initial_capital, transaction_cost_percentage, minimum_hold_days=3, gate_threshold=None):
    """
    Otimiza thresholds usando score s = P(up) - P(down).
    ...
    Returns:
        Tupla com (best_buy_threshold, best_sell_threshold, best_sharpe_ratio)
    """
    coarse_buy_grid = np.arange(0.05, 0.50, 0.05)
    coarse_sell_grid = np.arange(-0.50, 0.05, 0.05)

    best_buy_coarse, best_sell_coarse, _ = optimize_trading_thresholds_financial_with_score(
        probabilities, validation_dataframe, initial_capital, transaction_cost_percentage, 
        buy_threshold_grid=coarse_buy_grid, sell_threshold_grid=coarse_sell_grid, 
        minimum_hold_days=minimum_hold_days, gate_threshold=gate_threshold
    )

    fine_buy_grid = np.arange(0.10, 0.40, 0.02)
    fine_sell_grid = np.arange(-0.40, -0.05, 0.02)

    return optimize_trading_thresholds_financial_with_score(
        probabilities, validation_dataframe, initial_capital, transaction_cost_percentage, 
        buy_threshold_grid=fine_buy_grid, sell_threshold_grid=fine_sell_grid, 
        minimum_hold_days=minimum_hold_days, gate_threshold=gate_threshold
    )
...
def optimize_trading_thresholds_financial_with_probs(probabilities, validation_dataframe, initial_capital, transaction_cost_percentage, buy_threshold_grid=None, sell_threshold_grid=None, minimum_hold_days=3, gate_threshold=None):
    """
    ...
    Args:
        ...
        initial_capital: Capital inicial
        transaction_cost_percentage: Custo de transação
        buy_threshold_grid: Grade de thresholds de compra
        sell_threshold_grid: Grade de thresholds de venda
        minimum_hold_days: Mínimo de dias entre mudanças
        gate_threshold: Threshold de confiança opcional
...
def optimize_trading_thresholds_financial_with_score(probabilities, validation_dataframe, initial_capital, transaction_cost_percentage, buy_threshold_grid=None, sell_threshold_grid=None, minimum_hold_days=3, gate_threshold=None):
    """
    Otimiza thresholds de compra e venda usando score s = P(up) - P(down).
    ...
    """
    if buy_threshold_grid is None:
        buy_threshold_grid = SCORE_BUY_GRID
    if sell_threshold_grid is None:
        sell_threshold_grid = SCORE_SELL_GRID

    score_vec = compute_up_down_score_from_proba(probabilities)

    best_sharpe_ratio = -np.inf
    best_threshold_pair = (0.25, -0.05)
...
def _execute_buy_order(cash, stocks_held, execution_price, transaction_cost_pct, 
    ...
    """
    Executa ordem de compra com todos os custos e restrições.
    ...
    Args:
        ...
        sizing_config: Configuração do position sizing
        probability: Probabilidade de predição
        volume: Volume negociado no dia
        volatility: Volatilidade do dia
        slippage_config: Configuração do slippage
    ...
    Returns:
        Tupla com (new_cash, new_stocks_held, trade_executed, trade_cost)
    """
...
def _execute_sell_order(cash, stocks_held, execution_price, transaction_cost_pct, 
    ...
    Returns:
        Tupla com (new_cash, new_stocks_held, trade_executed, trade_value)
    """
    dynamic_slippage_bps = calculate_dynamic_slippage(
...
def run_realistic_backtest(test_dataframe, predictions, probabilities, initial_capital, transaction_cost_percentage, backtest_configuration=None):
    ...
    for day_index in range(len(test_dataframe) - 1):
        ...
        if current_prediction == 1 and available_cash > 0:
...
def run_profit_techniques_backtest(test_dataframe, predictions, probabilities, initial_capital, backtest_configuration=None):
    """
    Executa backtest simplificado sem custos de transação.
    
    Args:
        test_dataframe: DataFrame com dados OHLCV
        predictions: Array de predições binárias
...
def calculate_portfolio_metrics(portfolio_value_history, strategy_label, ticker_symbol, trading_actions=None):
    ...
    Args:
        ...
        strategy_label: Nome da estratégia
        ...
        for ticker in sorted(ticker_results.keys()):
            ...
            for result in ticker_results[ticker]:
                ...
                if 'ml_metrics' in result:
                    file.write(f"Métricas de ML ({result['label']}):\n")
                    file.write("-" * 40 + "\n")
...
def _group_results_by_ticker(results_data):
    ...
    Returns:
        Dicionário com ticker como chave e lista de resultados como valor
    """
    ticker_results = {}
...
def _write_summary_statistics(file, results_data):
    ...
    Args:
        ...
        results_data: Lista de dicionários com resultados
    """
    model_p_up_results = [r for r in results_data if r['label'] == 'Modelo P(up) Realista']
    model_score_results = [r for r in results_data if r['label'] == 'Modelo Score Realista']
    ...
    if model_p_up_results:
        avg_model_p_up_return = sum(r['retorno_total'] for r in model_p_up_results) / len(model_p_up_results)
        file.write(f"Retorno médio do modelo P(up) Realista: {avg_model_p_up_return:+.2f}%\n")
    
    if model_score_results:
        avg_model_score_return = sum(r['retorno_total'] for r in model_score_results) / len(model_score_results)
        file.write(f"Retorno médio do modelo Score Realista: {avg_model_score_return:+.2f}%\n")
    ...
    if model_p_up_results and model_score_results:
        file.write("\nCOMPARAÇÃO ENTRE ESTRATÉGIAS:\n")
        file.write("-" * 40 + "\n")
        
        p_up_returns = [r['retorno_total'] for r in model_p_up_results]
        score_returns = [r['retorno_total'] for r in model_score_results]
        
        p_up_wins = sum(1 for p, s in zip(p_up_returns, score_returns) if p > s)
        score_wins = sum(1 for p, s in zip(p_up_returns, score_returns) if s > p)
        
        file.write(f"Vitórias P(up): {p_up_wins} tickers\n")
        file.write(f"Vitórias Score: {score_wins} tickers\n")
        
        if p_up_wins > score_wins:
            file.write("Estratégia P(up) teve melhor performance geral\n")
        elif score_wins > p_up_wins:
            file.write("Estratégia Score teve melhor performance geral\n")
        else:
            file.write("Estratégias empataram\n")
    
    file.write(f"\nTotal de tickers analisados: {len(set(r['ticker'] for r in results_data))}\n")
...
def generate_json_report(results_data, backtest_configuration, output_path):
    ...
    report_data = {
        ...
        'results_by_ticker': {},
        'summary': {
            'model_p_up_performance': {},
            'model_score_performance': {},
            'buyhold_performance': {},
    ...
    for ticker, results in ticker_results.items():
        report_data['results_by_ticker'][ticker] = {
            'model_p_up_realistic': next((r for r in results if r['label'] == 'Modelo P(up) Realista'), None),
            'model_score_realistic': next((r for r in results if r['label'] == 'Modelo Score Realista'), None),
            ...
            'ml_metrics_p_up': next((r for r in results if r['label'] == 'Métricas de ML P(up)'), None),
            'ml_metrics_score': next((r for r in results if r['label'] == 'Métricas de ML Score'), None)
        }
...
def _calculate_performance_statistics(report_data, results_data):
    ...
    if model_p_up_results:
        report_data['summary']['model_p_up_performance'] = {
            'average_return': sum(r['retorno_total'] for r in model_p_up_results) / len(model_p_up_results),
            'best_performer': max(model_p_up_results, key=lambda x: x['retorno_total']),
            'worst_performer': min(model_p_up_results, key=lambda x: x['retorno_total']),
            'total_return': sum(r['retorno_total'] for r in model_p_up_results)
        }
    
    if model_score_results:
        report_data['summary']['model_score_performance'] = {
            'average_return': sum(r['retorno_total'] for r in model_score_results) / len(model_score_results),
            'best_performer': max(model_score_results, key=lambda x: x['retorno_total']),
            'worst_performer': min(model_score_results, key=lambda x: x['retorno_total']),
            'total_return': sum(r['retorno_total'] for r in model_score_results)
        }
    ...
    if model_p_up_results and model_score_results:
        ...
        report_data['summary']['strategy_comparison'] = {
            'p_up_wins': sum(1 for p, s in zip(p_up_returns, score_returns) if p > s),
            'score_wins': sum(1 for p, s in zip(p_up_returns, score_returns) if s > p),
            'p_up_avg_return': sum(p_up_returns) / len(p_up_returns),
            'score_avg_return': sum(score_returns) / len(score_returns)
        }
...
def _process_single_ticker(ticker_symbol, features_path, model_path, config, backtest_configuration):
    ...
    Returns:
        Tupla com (results_list, simulation_dataframe) ou (None, None) se erro
    """
    ...
    print(f"  Otimizando thresholds para estratégia P(up)...")
    ...
    print(f"  Otimizando thresholds para estratégia Score...")
    score_buy_threshold, score_sell_threshold, best_score_sharpe = adaptive_threshold_optimization_with_score(
        val_probs, validation_dataframe, backtest_configuration['initial_capital'],
        backtest_configuration['transaction_cost_pct'], minimum_hold_days=hold_min_days, gate_threshold=gate_threshold
    )
    
    print(f"  [VAL] Thresholds P(up): buy={buy_threshold:.3f}, sell={sell_threshold:.3f}, Sharpe={best_sharpe:.3f}")
    print(f"  [VAL] Thresholds Score: buy={score_buy_threshold:.3f}, sell={score_sell_threshold:.3f}, Sharpe={best_score_sharpe:.3f}")
    ...
    predictions_p_up = generate_actions_from_prob(
    ...
    predictions_score = generate_actions_from_score(
        score_sim, buy_threshold=score_buy_threshold, sell_threshold=score_sell_threshold, minimum_hold_days=hold_min_days
    )
    
    contagem_acoes_p_up = np.sum(predictions_p_up)
    contagem_acoes_score = np.sum(predictions_score)
    print(f"  [SIM] Ações P(up): {contagem_acoes_p_up} de {len(predictions_p_up)} dias")
    print(f"  [SIM] Ações Score: {contagem_acoes_score} de {len(predictions_score)} dias")
    
    realistic_portfolio_p_up = run_realistic_backtest(
        simulation_dataframe, predictions_p_up, p_up, backtest_configuration['initial_capital'],
        backtest_configuration['transaction_cost_pct'], backtest_configuration
    )

    realistic_portfolio_score = run_realistic_backtest(
        simulation_dataframe, predictions_score, score_sim, backtest_configuration['initial_capital'],
        backtest_configuration['transaction_cost_pct'], backtest_configuration
    )

    simple_portfolio = run_profit_techniques_backtest(
        simulation_dataframe, predictions_p_up, score_sim, backtest_configuration['initial_capital'], backtest_configuration
    )
    ...
    model_metrics_p_up = calculate_portfolio_metrics(realistic_portfolio_p_up, "Modelo P(up) Realista", ticker_symbol, predictions_p_up)
    model_metrics_score = calculate_portfolio_metrics(realistic_portfolio_score, "Modelo Score Realista", ticker_symbol, predictions_score)
    simple_metrics = calculate_portfolio_metrics(simple_portfolio, "Simulação Simples", ticker_symbol, predictions_p_up)
    buy_and_hold_metrics = calculate_portfolio_metrics(buy_and_hold_portfolio, "Buy and Hold", ticker_symbol)
    
    if config['model_training']['target_column'] in simulation_dataframe.columns:
        ml_metrics_p_up = calculate_ml_metrics(simulation_dataframe[config['model_training']['target_column']], predictions_p_up)
        ml_metrics_score = calculate_ml_metrics(simulation_dataframe[config['model_training']['target_column']], predictions_score)
    else:
        try:
            ...
            if config['model_training']['target_column'] in labeled_simulation.columns:
                min_size = min(len(labeled_simulation), len(predictions_p_up))
                target_values = labeled_simulation[config['model_training']['target_column']].iloc[:min_size]
                predictions_p_up_aligned = predictions_p_up[:min_size]
                predictions_score_aligned = predictions_score[:min_size]
                ml_metrics_p_up = calculate_ml_metrics(target_values, predictions_p_up_aligned)
                ml_metrics_score = calculate_ml_metrics(target_values, predictions_score_aligned)
                print(f"  Métricas de ML calculadas usando dados labeled: {len(target_values)} registros")
            else:
                ml_metrics_p_up = {'accuracy': 0, 'precision': 0, 'recall': 0, 'f1': 0}
                ml_metrics_score = {'accuracy': 0, 'precision': 0, 'recall': 0, 'f1': 0}
    ...
    ml_metrics_entry_p_up = {
        'ticker': ticker_symbol,
        'label': 'Métricas de ML P(up)',
        'ml_metrics': ml_metrics_p_up
    }
    
    ml_metrics_entry_score = {
        'ticker': ticker_symbol,
        'label': 'Métricas de ML Score',
        'ml_metrics': ml_metrics_score
    }
    
    results_dataframe = pd.DataFrame({
        'Model_P_up_realistic': realistic_portfolio_p_up,
        'Model_Score_realistic': realistic_portfolio_score,
        'Model_strategy_simple': simple_portfolio,
        'Buy_and_Hold': buy_and_hold_portfolio
    })

    return [model_metrics_p_up, model_metrics_score, simple_metrics, buy_and_hold_metrics, ml_metrics_entry_p_up, ml_metrics_entry_score], results_dataframe
...
def _load_configuration():
    ...
    Returns:
        Tupla com (config, backtest_config, features_path, model_path)
    """
